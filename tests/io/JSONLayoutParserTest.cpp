...
TEST(JSONLayoutParserTest, ParsesValidLayoutContainingOneShape) {
    ...
    EXPECT_EQ(shapes[0].getLayer(), Layer::M1);
...
TEST(JSONLayoutParserTest, LayerParserTest) {
    ...
    EXPECT_EQ(shapes[0].getLayer(), Layer::M1);
    EXPECT_EQ(shapes[1].getLayer(), Layer::M2);
    EXPECT_EQ(shapes[2].getLayer(), Layer::PO);
    EXPECT_EQ(shapes[3].getLayer(), Layer::OD);
    EXPECT_EQ(shapes[4].getLayer(), Layer::VIA1);
}
...
TEST(JSONLayoutParserTest, EmptyShapesArray) { const std::filesystem::path path =
        std::filesystem::path(DRCHECK_SOURCE_DIR) / "examples" / "empty_layout.json";
