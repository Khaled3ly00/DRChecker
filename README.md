...
DRCheck is a C++ Design Rule Checker for IC layouts. The project focuses on modern C++, computational geometry, IC verification concepts, unit testing, spatial indexing, and evidence-based performance optimization.
...
- select a named GDSII top-level cell when a library contains multiple top-level cells;
...
- optionally build a Qt/QML desktop GUI that runs the same DRC workflow without replacing the command-line interface;
...
- provide an interactive, paginated SVG violation browser with layer visibility, selective overlays, hover details, and cursor-centered zoom; and
- inspect normalized layouts in the desktop GUI with runtime layer controls, selectable violations, highlighted markers, panning, zooming, and fit-to-view navigation.

## Current Status

The geometry core, shape model, runtime `Layer`/`LayerRegistry` domain, direct GDSII and JSON layout input, Clipper2-backed layout normalization, rule framework, centralized `RuleFactory`, JSON and Tcl rule-deck input paths, reusable `DRCRunner`, Tcl automation runner and commands, dual-mode CLI, optional Qt/QML desktop GUI, interactive SVG violation browser, layer-aware violation metadata, QuadTree optimization, reusable spatial-index architecture, density-rule pipeline, and cross-platform CI workflow are implemented. Layers are no longer compiled into DRCheck: each rule deck declares the logical names available to its layout and rules, associates them with exact GDSII layer/datatype pairs, and supplies their shared identities through one registry for the complete run. The core C++ behavior is covered by unit, integration, and end-to-end tests. An optional Cadence SKILL utility can still produce compatible JSON, including the physical cut and enclosure geometry generated by supported standard vias.

`DRCRunner` selects `JSONLayoutParser` for `.json` layouts and the gdstk-backed `GDSLayoutParser` for `.gds` or `.gdsii` layouts. The GDSII path converts coordinates to micrometers, assigns deterministic sequential shape IDs, imports polygon and path geometry from the top-level cell and its nested references, expands SREF and AREF hierarchy through all levels, and applies reference translation, rotation, reflection, magnification, and repetition. A library with one top-level cell is selected automatically; a library with multiple top-level cells requires an explicit name through CLI `--top` or Tcl `drc_run -top`. Recursive cell-reference cycles and missing or ambiguous top-level selections are rejected.
...
Complete DRC execution is separated from `main()`. `DRCRunner` owns layout loading, rule-deck selection, engine execution, and report generation, allowing direct CLI runs, Tcl automation commands, and the desktop GUI to share one workflow.

The optional `drcheck_gui` executable calls `DRCRunner` directly and displays the returned normalized shapes and violations; it does not launch the CLI or reconstruct the layout from the JSON report. The Qt-facing controller and models remain outside the Qt-free `drchecker` core library.
...
| `domain/` | Runtime layer declarations, canonical identity, exact GDS stream mappings, shapes, violations, and violation markers |
...
| `io/` | JSON and GDSII layout parsing, JSON/Tcl rule-deck parsing, Tcl automation, parameter conversion, JSON reporting, and interactive SVG generation |
| `gui/` | Optional Qt controller and list/view models that expose completed DRC results to QML |
| `qml/` | Desktop application shell, layout canvas, layer controls, and violation-list delegates |
| `scripts/virtuoso/` | Optional Cadence SKILL adapter that exports layout geometry to compatible DRCheck JSON |

## Layout Inputs and Layers

### JSON layouts

`JSONLayoutParser` is DRCheck's JSON layout-input boundary. It accepts layouts from hand-written files, generators, EDA exporters, or any other producer that emits the documented schema. Layouts use micrometers and require a top-level `units` field set to `"um"` plus a `shapes` array. Every shape must provide an integer `id`, a declared `layer`, and polygon `vertices`:
...
There is no compiled-in layer vocabulary. The selected JSON or Tcl rule deck defines the valid logical names for a run, and `JSONLayoutParser` resolves every shape name through that deck's `LayerRegistry`. Layer declarations now include one or more exact GDSII mappings even when the selected layout is JSON. For example, a layout containing `M1` is valid when its JSON deck declares an `M1` layer object with a non-empty `gdsMappings` array or its Tcl deck evaluates `layer M1 -map {-layer 31 -datatype 0}` before rules that use it. This permits different technologies and naming conventions without changing C++ source code.
...
`GDSLayoutParser` uses gdstk to read `.gds` and `.gdsii` files directly. The rule deck is loaded first so every imported stream pair can be resolved to the same canonical logical `Layer` objects used by rules, normalization, indexing, and reports. One logical layer can select several exact GDSII pairs, while each individual pair currently maps to exactly one logical layer.

JSON layer declarations use a non-empty `gdsMappings` array:
...
Tcl uses one repeatable `-map` list per exact pair:

```tcl
layer M1 \
    -map {-layer 31 -datatype 0} \
    -map {-layer 31 -datatype 1}

layer VIA1 -map {-layer 51 -datatype 0}
```

Layer and datatype values must be non-negative integers. Empty mapping lists, malformed mapping objects, duplicate GDSII pairs, mappings to foreign registry objects, and imported pairs that have no mapping are rejected with descriptive errors.

For geometry, the parser:

- imports top-level polygons and converts GDSII paths to polygons both at the top level and inside direct, nested, and array references, including flush, extended, and round path ends;
- removes a repeated closing vertex before constructing the internal polygon;
- expands SREF and AREF geometry through all nested reference levels;
- applies translation, rotation, X-axis reflection, magnification, and array repetition;
- detects recursive cell references before hierarchy expansion;
- converts coordinates to micrometers; and
- assigns deterministic sequential IDs in import order.

When the file contains exactly one top-level cell, it is selected automatically. When it contains multiple top-level cells, pass the desired cell through `--top <cellName>` in direct CLI mode or `-top <cellName>` in `drc_run`. The selected name must identify one of the library's top-level cells.

## Cadence Virtuoso Layout Export

Virtuoso is one optional source of layout data, not a required stage in the DRCheck pipeline. DRCheck can now read a compatible GDSII file directly, while the existing JSON path remains available for hand-written inputs, generators, and custom exporters. For Cadence users who want JSON output, the repository includes `scripts/virtuoso/layout_export.il`, a SKILL adapter that exports the active Virtuoso layout cellview into the documented JSON schema.
...
3. requiring each layer declaration to provide a name and non-empty `gdsMappings` array;
4. validating and registering each exact integer GDSII layer/datatype pair;
5. resolving every rule layer reference to its canonical registry-owned `Layer`;
6. converting density limits from `Minimum` or `Maximum`;
7. parsing optional analysis-window bounds from `minX`, `minY`, `maxX`, and `maxY`;
8. decoding enclosure-option objects and requiring opposite-pair values together;
9. populating `RuleParameters`; and
10. mapping the external JSON type to the internal factory key and delegating construction.
...
rules.json
       ...
       v
JSONRuleParser
layer declarations, GDS mappings, and field validation
       |
       +------> LayerRegistry
       |
       v
...
{
  "layers": [
    {"name": "VIA1", "gdsMappings": [{"layer": 51, "datatype": 0}]},
    {"name": "M1", "gdsMappings": [{"layer": 31, "datatype": 0}]}
  ],
  "rules": [
    {
      "type": "MinEnclosure",
...
{
  "layers": [
    {"name": "CO", "gdsMappings": [{"layer": 30, "datatype": 0}]},
    {"name": "M1", "gdsMappings": [{"layer": 31, "datatype": 0}]},
    {"name": "PO", "gdsMappings": [{"layer": 17, "datatype": 0}]}
  ],
  "rules": [
    {
      "type": "MinEnclosure",
...
`TclRuleParser` adds a second rule-deck front end without changing the rule or engine interfaces. Its static `load()` function creates a Tcl interpreter, registers the DRCheck-specific `layer` and `rule` object commands, evaluates the requested file, and returns the resulting polymorphic rules. Each layer declaration requires one or more exact GDSII mappings and must appear before the first rule that references it; duplicate declarations, duplicate stream pairs, malformed maps, and undeclared rule-layer references are rejected.

The supported Tcl declarations mirror the four JSON rule families:

```tcl
layer M1 -map {-layer 31 -datatype 0} -map {-layer 31 -datatype 1}
layer M2 -map {-layer 32 -datatype 0}
layer VIA1 -map {-layer 51 -datatype 0}
...
layer declaration, GDS mapping, option validation, and conversion
    |
    +------> LayerRegistry
    |
    v
...
Completed execution is returned as one owning result:
...
2. selects `JSONRuleParser` or `TclRuleParser` from the rule-deck extension, then loads logical-layer declarations, GDSII mappings, and rules into that registry;
3. selects `JSONLayoutParser` or `GDSLayoutParser` from the layout extension and resolves every imported shape through the same registry;
...
8. returns the registry, normalized shapes, and generated violations in a `DRCRunResult`.

`DRCRunResult` owns its `LayerRegistry` through `std::unique_ptr`, making the completed result move-only in normal use. The returned shapes and violation-marker layer fields contain non-owning pointers to registry-owned `Layer` objects, so callers that inspect those objects must retain the complete result for as long as the pointers are used. This extends the required registry lifetime beyond rule execution and report generation and allows the GUI to safely retain and display one completed run.

This same function is used by direct CLI mode, Tcl automation, and the desktop GUI, so parsing, execution, normalization, and report behavior are not duplicated in any front end.

`TclAutomationRunner::run()` creates a Tcl interpreter with two DRCheck-specific commands:

| Command | Parameters | Behavior |
|---|---|---|
| `drc_run` | required `-layout`, `-rules`, `-report`; optional `-svg`, `-top` | Builds a `DRCRunConfig`, optionally selects a GDSII top-level cell, executes the complete workflow, and stores the resulting violations as the latest run |
...
DRCRunner
  ...
  -> load JSON/Tcl logical layers, GDS mappings, and rules
  -> load JSON or GDSII layout through the same registry
  -> normalize connected same-layer geometry
  -> DRCEngine
  -> JSON report
  -> optional SVG report
  -> DRCRunResult { LayerRegistry, normalized shapes, violations }
```
...
`Shape` combines a unique ID, a non-null layer pointer, and an octilinear polygon. Construction rejects a null layer or any polygon edge outside the supported horizontal, vertical, and 45-degree directions. Rules, enclosure options, violation markers, and `LayerSpatialIndex` use the same pointer identity. These pointers are non-owning, so the `LayerRegistry` must outlive every object produced from it; `DRCRunner` returns the registry in the same owning `DRCRunResult` as the normalized shapes and violations.
...
`ViolationMarker` supports optional witness points, polygon-edge indices, and a region. `MinWidthRule` stores two witness points plus the two corresponding edge indices. `MinSpacingRule` and measured `MinEnclosureRule` failures store their two closest witness points while leaving `firstEdgeIndex` and `secondEdgeIndex` unset. A missing-enclosure failure stores the unenclosed inner polygon's bounding box as a region, and window-based density violations store the evaluated window as a region. The marker also carries optional non-owning `firstLayer` and `secondLayer` pointers so a report can describe the physical layer or layer pair independently of shape lookup. Width, spacing, enclosure, and density rules populate the applicable layer fields.
...
An unenclosed inner shape produces an `Enclosure` violation with only its own shape ID, a layer-specific message such as `PO is not enclosed by any allowed outer layer`, and `actual = 0.0`, `required = 0.0`. Its marker identifies `firstLayer` and uses the inner polygon's bounding box as a region so JSON, SVG, and GUI consumers can locate the failure even though there is no closest-point pair. Those zero values describe the missing-enclosure case, not a configured zero threshold; consumers should inspect the message and participating IDs.
...
## Optional Qt/QML Desktop GUI

DRCheck also provides an optional `drcheck_gui` executable for running DRC and inspecting the result in a desktop application. It is a presentation layer over the same `drchecker` library and `DRCRunner` workflow used by the CLI and Tcl automation; the core geometry, rule, parser, normalization, and report code remains independent of Qt.
...
layout.json / layout.gds / layout.gdsii
       ...
       |                    |
       v                    v
folder/report.json     DRCRunResult
                       registry + normalized shapes + violations
                                  |
                     +------------+-------------+
                     |            |             |
                     v            v             v
              LayerListModel LayoutViewModel ViolationListModel
                     |            |             |
                     +------------+-------------+
                                  |
                                  v
...
The toolbar selects a JSON or GDSII layout, a JSON or Tcl rule deck, and the directory that will receive `report.json`. The Run button becomes available after all three inputs are selected. A new run clears the previous models and marker before execution; successful completion repopulates the viewer, while parser, validation, or DRC exceptions are caught at the controller boundary, clear partial state, and appear in the status bar instead of terminating the application.

The main window displays the normalized physical polygons returned by `DRCRunner`, rather than the unmerged source figures or a reconstruction of the JSON report. The QML `Canvas` fits the complete layout while preserving aspect ratio and reversing the layout Y axis for screen coordinates. Navigation supports cursor-centered mouse-wheel zoom, centered `Zoom +`/`Zoom -` buttons, middle-button drag panning, and fit-to-view through either the `Fit` button or the `F` shortcut.

Runtime layer controls are derived from the logical layers present in the normalized shapes. Each populated layer appears once in an alphabetically ordered, scrollable table with its name, color, and visibility toggle. Layers start visible with generated colors. Individual layers can be hidden or shown without rerunning DRC, `Hide All`/`Show All` changes the complete layer set, and clicking a color swatch opens a shared color picker that updates both the table and canvas immediately.

The violation panel provides a scrollable row for each result. A compact summary contains the violation type, available marker layer or layer pair, and `actual/required` values; selecting a row shows its complete message in a wrapped, scrollable details panel. Clicking the selected row again deselects it. Point-based markers are drawn as a red measurement line and witness circles, while density and missing-enclosure regions are drawn as translucent red rectangles with an opaque border. Selecting a marker automatically centers it without changing the current zoom level.

The GUI bridge is split by responsibility:

- `AppController` owns input paths, run/error state, selected-violation coordination, and the current `DRCRunResult` lifetime;
- `LayoutViewModel` converts normalized polygons and marker geometry into QML-facing data and exposes the layout bounds;
- `LayerListModel` owns layer names, display colors, visibility, and the all-visible state;
- `ViolationListModel` exposes violation summaries and full messages; and
- reusable `LayoutView.qml`, `LayerItem.qml`, and `ViolationItem.qml` components keep rendering and delegate behavior outside the application shell.
...
`DRCRunner` composes the stateless static entry points `JSONRuleParser::load()` or `TclRuleParser::load()`, `JSONLayoutParser::load()` or `GDSLayoutParser::load()`, `LayoutNormalizer::normalize()`, `DRCEngine::run()`, and the report writers. It returns the completed registry, normalized shapes, and violations through `DRCRunResult`. Direct CLI mode, Tcl automation, and the desktop GUI therefore converge on the same declare-parse-normalize-check-report path for both supported layout formats.

## Project Structure

```text
drcheck/
├── include/drcheck/, src/          # C++ headers and sources
...
│   ├── io/                         # JSON/GDSII layout, JSON/Tcl rules, reports, Tcl automation
│   └── gui/                        # optional Qt controller and QML-facing models
├── layout/                         # LayoutNormalizer (Clipper2-based polygon union)
├── qml/                            # optional desktop GUI views and delegates
...
- Qt 6.8 or newer with Qt Quick and Qt Quick Controls 2 only when building the optional GUI; and
- Git when dependencies are fetched by CMake.

CMake fetches `nlohmann/json` `3.12.0`, Clipper2 `2.0.1`, gdstk `1.0.1`, zlib `1.3.2`, and Qhull `8.0.2`. DRCheck links gdstk for GDSII input and limits dependency-side examples, applications, utilities, and tests where the upstream builds permit it.
...
### Building the optional GUI

`DRCHECK_BUILD_GUI` defaults to `OFF`, so normal library, CLI, test, and CI configurations do not require Qt. To add the `drcheck_gui` target, install Qt 6.8 or newer with Qt Quick and Qt Quick Controls 2, then enable the option:

```bash
cmake -S . -B build-gui -DDRCHECK_BUILD_GUI=ON -DCMAKE_PREFIX_PATH="/path/to/Qt/compiler-kit"
cmake --build build-gui --config Release --target drcheck_gui
```

`CMAKE_PREFIX_PATH` can be omitted when CMake already discovers the selected Qt kit. The value should name the kit-specific Qt prefix that contains `lib/cmake/Qt6`; alternatively, configure the project with a Qt Creator kit and add `DRCHECK_BUILD_GUI=ON` to that configuration. In Qt Creator, select `drcheck_gui` as the run target—the `drcheck` target remains the command-line program and expects CLI arguments.
...
The current CI matrix uses the default `DRCHECK_BUILD_GUI=OFF`, does not install Qt, and therefore validates the core library, CLI, and automated C++ suite but not the optional GUI target or QML behavior.

## Running DRCheck

### Desktop GUI

After building with `DRCHECK_BUILD_GUI=ON`, launch the generated `drcheck_gui` executable from the selected build configuration. For example, a single-configuration build commonly uses:

```bash
./build-gui/drcheck_gui
```

and a Windows multi-configuration Release build commonly uses:

```powershell
.\build-gui\Release\drcheck_gui.exe
```

Use `Open Layout` to select a `.json`, `.gds`, or `.gdsii` layout, `Open Rules` to select a `.json` or `.tcl` deck, and `Report Folder` to select the output directory. `Run DRC` executes the same normalized DRC pipeline as the CLI and writes `report.json` in that directory. The resulting layout, layers, violation list, details, and available marker geometry remain in the application for interactive inspection.

### Command-line interface

The CLI uses named argument/value pairs, which can appear in any order. `--layout`, `--rules`, and `--report` are required, while `--svg` and `--top` are optional. The rule-deck parser is selected from the `.json` or `.tcl` extension; the layout parser is selected from `.json`, `.gds`, or `.gdsii`.

The selected rule deck must declare every logical layer used by the layout and provide at least one exact GDSII layer/datatype mapping for each declaration. `DRCRunner` loads the deck first so the same registry can resolve rule references, JSON layer names, and GDSII stream pairs.
...
Run a GDSII layout through the same normalization, rule, and report pipeline:

```bash
drcheck --layout layout.gds --rules rules.tcl --report report.json --svg report.svg
```

If the GDSII library contains multiple top-level cells, select one explicitly:

```bash
drcheck --layout layout.gds --top TOP_CELL --rules rules.tcl --report report.json --svg report.svg
```

`--top` is rejected for JSON layouts. Without `--top`, a GDSII file must contain exactly one top-level cell.
...
The direct CLI and Tcl automation flow is:
  ...
  DRCRunConfig                        TclAutomationRunner
        ...
        +--------------> DRCRunner <------------+
                  ...
                  JSON/Tcl rule deck
                               |
                               v
                  declare logical layers
                  + exact GDS mappings
                    + construct rules
                               |
                               v
                parse JSON or GDSII layout
                  + resolve logical layers
                    + layout normalization
                               |
                               v
                           DRCEngine
...
This exercises GDSII parsing, stream-pair resolution through the rule deck, hierarchy expansion, normalization, DRC execution, and both report writers without using the Virtuoso-to-JSON adapter.
...
- Direct layout import supports DRCheck's polygon JSON schema and GDSII, but not other layout formats. JSON input units must be `"um"`.
- GDSII import does not import text, labels, or layout records other than polygons and paths.
...
- The desktop GUI writes a fixed `report.json` in the selected folder and does not currently expose SVG generation, a custom report filename, or GDSII top-level-cell selection. A GDSII library with multiple top-level cells must therefore be run through the CLI or Tcl interface with an explicit top-cell name.
- GUI DRC execution currently runs synchronously on the application thread and has no progress or cancellation control. The QML Canvas redraws the polygon collection when navigating or changing layer styles, and responsiveness has not yet been characterized for very large layouts.
...
- GUI hardening through background execution and progress reporting, richer run/output controls, persistent display preferences, large-layout profiling, and dedicated Qt/QML test coverage;
...
- runtime, rule-deck-defined logical layers with stable identity and exact GDSII layer/datatype mapping across parsers, rules, geometry, indexes, and reports;
...
- interactive, standalone SVG inspection;
- an optional Qt/QML desktop workflow that reuses the core DRC pipeline and completed-run data; and
